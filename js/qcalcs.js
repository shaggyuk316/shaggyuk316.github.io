...
function simulateQCalcs(n, t_start = 1e-108, e_base = 1e-93, atomicNumber, mass, vibration, charge, density, sound, magnetic) {
    const q_calcs = [];
    const k_v = 1e-50, k_q = 1e-50, f_s = 1e95;
    const V_s = vibration ? k_v * vibration * Math.sqrt(mass) : 0;
    const q = charge;
    const rho = density || 0.1;
    const s = sound || 0;
    const m = magnetic || 0;
    for (let i = 1; i <= n; i++) {
        const t = t_start * Math.pow(10, (i - 1) / 9);
        const r = Math.random();
        const e = atomicNumber * Math.sqrt(mass * e_base * r) * Math.PI * (V_s + k_q * q + rho + s + m);
...
function updateQCalcs() {
    ...
    const density = densityToggle ? parseFloat(document.getElementById("density").value) : 0;
    const soundToggle = document.getElementById("soundToggle").checked;
    const sound = soundToggle ? parseFloat(document.getElementById("sound").value) : 0;
    const magneticToggle = document.getElementById("magneticToggle").checked;
    const magnetic = magneticToggle ? parseFloat(document.getElementById("magnetic").value) : 0;

    document.getElementById("element-info").textContent = `Element: ${element.symbol} (Z: ${element.atomicNumber}, m: ${element.mass})`;
    const table = document.getElementById("qcalcsTable");
    while (table.rows.length > 1) table.deleteRow(1);
    const q_results = simulateQCalcs(10, 1e-108, 1e-93, element.atomicNumber, element.mass, vibration, charge, density, sound, magnetic);
    ...
    function animate() {
        ...
        spheres.forEach(sphere => {
            const scale = (V_s || q || rho || s || m) ? 1 + Math.sin(Date.now() * 0.001) * (sphere.baseEnergy * 1e50) : 1;
